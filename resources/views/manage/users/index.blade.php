...
@section('main-content')
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <div class="table-responsive">
                                <table id="example44546" class="wide display table table-striped table-hover nowwrap"
                                    style="width:100%">
                                    <thead>
                                        <tr>
                                            <th><i>#</i></th>
                                        ...
                                        @foreach ($users as $user)
                                            <tr>
                                                ...
                                                <td class="col-dt-hidden noExport">
                                                    ...
                                                    {{-- @php
                                                    $profileImage = $user->profile->image ?? null;
                                                    $imageExists = $profileImage && Storage::disk('public')->exists('users/' . $profileImage);
                                                    $thumbnailExists = $profileImage && Storage::disk('public')->exists('users/thumbnails/' . $profileImage);
                                                
                                                    $imageUrl = $thumbnailExists
                                                        ? route('storage.file', ['folder' => 'users/thumbnails', 'filename' => $profileImage])
                                                        : ($imageExists
                                                            ? route('storage.file', ['folder' => 'users', 'filename' => $profileImage])
                                                            : ($profileImage
                                                                ? asset('storage/common/loading.gif')
                                                                : asset('storage/common/no_image.png')));
                                                @endphp --}}
    ...
    $imageUrl = $thumbnailExists
        ? route('storage.file', ['path' => 'users/thumbnails/' . $profileImage])
        : ($imageExists
            ? route('storage.file', ['path' => 'users/' . $profileImage])
            : ($profileImage
                ? asset('storage/common/loading.gif')
                : asset('storage/common/no_image.png')));
@endphp
