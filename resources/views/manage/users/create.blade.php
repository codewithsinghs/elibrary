...
@section('scripts')
    ...
    <!-- Working with autoselected, availabl options -->
    {{-- <script>
        document.addEventListener('DOMContentLoaded', function() {
            const categoryDropdown = document.getElementById('category');
            const instituteDropdown = document.getElementById('institute');
            ...
            function populateDropdown(dropdown, options, oldValue) {
                dropdown.innerHTML = '';
                options.forEach(optionValue => {
                    const option = document.createElement('option');
                    option.text = optionValue;
                    option.value = optionValue;
                    if (oldValue && oldValue === optionValue) {
            ...
            categoryDropdown.addEventListener('change', function() {
                const selectedCategory = this.value;
                const institutes = academics[selectedCategory]?.map(academic => academic.institute) || [];
                populateDropdown(instituteDropdown, [...new Set(institutes)], '{{ old('institute') }}');
                instituteDropdown.dispatchEvent(new Event('change')); // Trigger institute change event
            });
            ...
            instituteDropdown.addEventListener('change', function() {
                ...
                const faculties = academics[selectedCategory]
                    ?.filter(academic => academic.institute === selectedInstitute)
                    .map(academic => academic.faculty) || [];
                populateDropdown(facultyDropdown, [...new Set(faculties)], '{{ old('faculty') }}');
                facultyDropdown.dispatchEvent(new Event('change')); // Trigger faculty change event
            });
            ...
            facultyDropdown.addEventListener('change', function() {
                ...
                const departments = academics[selectedCategory]
                    ?.filter(academic => academic.institute === selectedInstitute && academic.faculty ===
                        selectedFaculty)
                    .map(academic => academic.department) || [];
                populateDropdown(departmentDropdown, [...new Set(departments)], '{{ old('department') }}');
                departmentDropdown.dispatchEvent(new Event('change')); // Trigger department change event
            });
            ...
            departmentDropdown.addEventListener('change', function() {
                ...
                const courses = academics[selectedCategory]
                    ?.filter(academic => academic.institute === selectedInstitute && academic.faculty ===
                        selectedFaculty && academic.department === selectedDepartment)
                    .map(academic => academic.course) || [];
                populateDropdown(courseDropdown, [...new Set(courses)], '{{ old('course') }}');
            });

            categoryDropdown.dispatchEvent(new Event('change')); // Trigger category change event
        });
    </script> --}}
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function populateDropdown(dropdown, options, oldValue, selectText = 'Select...') {
                dropdown.innerHTML = '';
                const defaultOption = document.createElement('option');
                defaultOption.text = selectText;
                defaultOption.value = '';
    ...
    {{-- <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function populateDropdown(dropdown, options, oldValue) {
                ...
                if (options.length === 0) {
                    // If there are no options, add a disabled "Select" option
                    const defaultOption = document.createElement('option');
                    defaultOption.text = 'Select ' + dropdown.id.charAt(0).toUpperCase() + dropdown.id.slice(1);
                    defaultOption.value = '';
                    ...
                    // Automatically select the oldValue if it exists in the options
