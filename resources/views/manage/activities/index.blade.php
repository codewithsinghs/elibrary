                        ...
                        @if (count($activities) > 0)
                            ...
                            <input type="text" id="roleSearch" placeholder="Role"> --}}
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <div class="input-group">
                                            ...
                                            <input type="date" id="start_date" name="start_date"
                                                class="datepicker-default form-control" placeholder="From Date">
                                        </div>
                                    </div>
                                    <div class="col-md-4">
                                        ...
                                        <div class="input-group">
                                            ...
                                            <input type="date" id="end_date" name="end_date"
                                                class="datepicker-default form-control" placeholder="To Date">
                                ...
                                <div class="table-responsive">
                                                    ...
                                                    <td>
                                                        {{-- @if ($activity->time_spent > 0)
                                                            @if (floor($activity->time_spent / 3600) > 0)
                                                                {{ floor($activity->time_spent / 3600) }} h
                                                            @endif
                                                        ...
                                                        $carbonInterval =  Carbon\Carbon::now()->($activity->time_spent)->diffForHumans(['parts' => 3]);
                                                    @endphp --}}
                                                        {{-- @php

                                                            $zero = Carbon\Carbon::now()->startOfDay();
                                                            $time = Carbon\Carbon::now()
                                                                ->startOfDay()
                                                                ->addSeconds($activity->time_spent);
                                                            $duration = $zero
                                                                ->diff($time)
                                                                ->format('%H hours, %I min, %S sec');
                                                        @endphp

                                                        {{ $duration }} --}}

                                                        {{-- {{ gmdate("H:i:s", $activity->time_spent); }}  --}}

                                                        {{-- @php
                                                           
                                                            $time = Carbon\Carbon::now();
                                                            $humanTime = $time->diffForHumans($time->copy()->addSeconds($activity->time_spent), true, false, 4);
                                                        @endphp
                                                        {{ $humanTime }} --}}
                                                     
                                                        {{-- {{ \Carbon\CarbonInterval::seconds($activity->time_spent)->cascade()->forHumans();}} --}}
                                                        {{ \Carbon\CarbonInterval::seconds($activity->time_spent)->cascade()->forHumans(['short' => true, 'parts' => 3]) }}
                                                      {{-- {{ \Carbon\Carbon::createFromTimestampUTC($activity->time_spent)->format('H:i:s')  }} --}}
                                                      
                                                        {{-- {{ DATEPARSE("s",[$activity->time_spent]) }} --}}
                                                        {{-- @php
                                                            $hours = floor($activity->time_spent / 3600);
                                                            $minutes = floor(($activity->time_spent % 3600) / 60);
                                                            $seconds = $activity->time_spent % 60;
                                                        @endphp

                                                        {{ sprintf('%02d hours, %02d min, %02d sec', $hours, $minutes, $activity->time_spent) }} --}}
                                                    </td>
                                                </tr>
                                            @endforeach
                                        </tbody>
                                    ...
                                    <div class="col-10">
                                        <nav>
                                            {{ $activities->onEachSide(2)->links('pagination::bootstrap-5') }}
                                        </nav>
                                    </div>
                                </div> --}}
                                </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#example14').DataTable({
                ...
                //     // Show only the desired number of page links
                //     var numPages = $('.paginate_button').length - 2; // Excluding 'Previous' and 'Next' buttons
                //     var showPages = 5; // Number of pages to show
                //     var currentPage = parseInt($('.paginate_button.current').text());
